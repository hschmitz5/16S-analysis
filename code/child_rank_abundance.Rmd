...
```
```{r stacked}
genus_size <- get_size_genus(ps_ASV) 
...
p <- ggplot(genus_size, aes(x = size.name, y = mean_ab, fill = fct_rev(Genus))) +
  ...
  scale_fill_manual(
    values = c("gray", met.brewer("Hiroshige", n_display)),
    name = "Genera"
  ) +
  scale_y_continuous(
...
genus_size <- get_size_genus(ps_ASV) %>%
  filter(Genus %in% high_ab_genera[1:n_display])   

p <- ggplot(genus_size, aes(x = Genus, y = mean_ab, fill = as.factor(size.mm))) +
     ...
     scale_fill_manual(
      values = met.brewer("Java", n_sizes),
      name = "Size [mm]"
     ) +
     scale_y_continuous(
       expand = expansion(mult = c(0, 0.1))  # Add 10% space above
...
# Save plot
fname <- "../figures/abund_size_error.png"
ggsave(fname, plot = p, width = 8, height = 4, dpi = 300)
...
#for (i in 1:length(high_ab_genera)) {
i <- 2

ps_rel_ASV <- get_rel_ASV(ps_ASV)

gs <- ps_rel_ASV %>%
  filter(Genus %in% high_ab_genera[i], OTU %in% OTU_names)
  gs %>%
    ...
    labs(
       title = high_ab_genera[i],
       x = "Sieve Range [mm]",
       y = "Relative Abundance [%]",
     ) +
    theme_minimal(base_size = 14)   # sets text size
  
  fname = paste0("../figures/genus_",high_ab_genera[i], ".png")
