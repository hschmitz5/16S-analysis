...
ps_rel_filt = filter_taxa(ps_rel, function(x) max(x) > 0.25, TRUE)  # for visualizing abundance only
...
print(paste("Number of ASVs in small mat:", ntaxa(ps_rel_filt)))
```
```{r genus_abundance}
...
top15 = head(sort(rowSums(otu_table(ps_rel)/nsamples(ps_rel)), decreasing = TRUE), 15) 
#top10 = head(sort(rowSums(otu_table(ps_rel_genus)/nsamples(ps)), decreasing = TRUE), 10) 

# Combine count and taxonomyTable
top15 = cbind(as.data.frame(tax_table(ps_rel)[names(top15),]), "Relative abundance [%]" = top15)
...
table_rel_filt <- as.data.frame(ps_rel_filt@otu_table) %>% 
  rownames_to_column(var = "seq")
``` 
...
tax_short0 <- taxonomy[,c(1,7,8)]  # keep seq, Genus, Species
tax_short  <- tax_short0[order(tax_short0$Genus), ]  # sort alphabetically
...
# Filters out more
table_rel_long_filt <- table_rel_filt %>% 
  pivot_longer(cols = !seq, names_to = "sample", values_to = "rel_ab") %>% # make a "long" dataframe
  left_join(taxonomy, join_by(seq)) %>% # join taxonomy by the sequence ID
  left_join(metadata, join_by(sample)) # join metadata by the sample ID

head(table_rel_long_filt)
```
...
genus_summary_filt <- table_rel_long_filt %>% # perform some functions on table_rel
...
# Test
sum(genus_summary[,4])/nsamples(ps_rel)
sum(genus_summary_filt[,5])/nsamples(ps_rel)

```
# Rank Abundance Curve
This shows the relative abundance, averaged for all samples, and their corresponding genera.
...
```{r rank_abundance_curve}
n_asv = 25  # number of ASVs to include in the plot
  
# I want to average across samples
rel_ab_avg <- rowMeans(table_rel_wide[,4:21])
table_rel_avg <- cbind(table_rel_wide[,1:3],rel_ab_avg)
order_idx <- order(table_rel_avg$rel_ab_avg, na.last = TRUE, decreasing = TRUE)  # indices
table_rel_avg_sorted = table_rel_avg[order_idx,]    
t <- head(table_rel_avg_sorted,n_asv)

# Factor Genera
fct_Genus <- forcats::fct_infreq(t$Genus, t$rel_ab_avg) # factor genus based on decreasing rel_ab
fct_levels <- levels(fct_Genus)

  p <- t %>%
       mutate(Genus = factor(Genus, levels = fct_levels)) %>%
       ggplot(aes(x = Genus, y = rel_ab_avg)) + 
       #ggplot(aes(x = forcats::fct_infreq(Genus, rel_ab_avg), y = rel_ab_avg)) + 
...
```
This groups per sample
Genus are conglomerated (not separated by ASVs)
```{r rank_abundance_curve2}
tmp <- head(table_rel_avg_sorted,n_asv)
genus_names <- unique(tmp$Genus)

gs <- genus_summary %>% # how to pipe data (dplyr filter)
    filter(Genus %in% genus_names) 
  
p <-  gs %>%
    mutate(Genus = factor(Genus, levels = fct_levels)) %>%
    ggplot(aes(x = Genus, y = sum_ab, fill = size.mm)) + 
    geom_bar(stat = "identity", position = "dodge") +
      xlab("Genus") +
      ylab("Relative Abundance [%]")  #
    p + theme(axis.text.x = element_text(angle = 45, hjust = 1))
    
    fname = paste("./results/genus/abund_sample.png") 
      ggsave(fname)

```
This one summarizes each sample individually
```{r rank_abundance_curve_single}

sam_names <- sample_names(ps)
for (i in 1:length(sam_names)) {
  test <- filter(table_rel_long, table_rel_long$sample == sam_names[i])   ## one at a time -- add for loop
  order_idx <- order(test$rel_ab, na.last = TRUE, decreasing = TRUE)  # indices
  test2 <- test[order_idx,] 
    
  p <- head(test2,20) %>%
       ggplot(aes(x = forcats::fct_infreq(Genus, rel_ab), y = log10(rel_ab))) +  # I think it's listing x in alphabetical order
       geom_point(
          shape = 21,
          color = "black",
          size = 3,
      ...
      labs(title = sam_names[i]) + 
        xlab("Genus") +
        ylab("log of relative abundance [%]")  #
      p + theme(axis.text.x = element_text(angle = 45, hjust = 1))
  
  fname = paste("./results/genus/abund_",sam_names[i], ".png", sep = "") # i
    ggsave(fname)
}
```
...
Summarizes genus abundance

# Single scatter plot
```{r plot_scatter_loop}
#for (i in 1:10) {
i = 5
  gs <- genus_summary_filt %>% # how to pipe data (dplyr filter)
  ...
  fname = paste("./results/genus/genus_",genus_names[i], ".png", sep = "") # i
  ggsave(fname)
#}
```
