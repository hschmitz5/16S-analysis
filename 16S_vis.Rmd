...
```{r sort_genus}
# Keep separate seq per genus  (collapse size)
genus_summary_seq <- table_rel_long %>%
  group_by(Genus, Species, seq) %>%
  summarise(avg_ab = sum(rel_ab)/nsamples(ps)) # average across samples

# Test that the average relative abundance adds up to 100%
sum(genus_summary_seq$avg_ab)
...
#### Subset the data to include n_asv ASVs:
# Order genus_summary_seq based on decreasing relative abundance
order_idx <- order(genus_summary_seq$avg_ab, na.last = TRUE, decreasing = TRUE)  
# Apply the indeces to sort the data
gs_sorted = genus_summary_seq[order_idx,] 
# Subset the data 
t <- head(gs_sorted,n_asv)

### Factor Genera
fct_Genus <- forcats::fct_infreq(t$Genus, t$avg_ab) # factor genus based on decreasing rel_ab
fct_Genus_levels <- levels(fct_Genus)
genus_names <- unique(t$Genus) # This is used in a later plots

### Factor seqs 
fct_seq <- forcats::fct_infreq(t$seq, t$avg_ab) # factor genus based on decreasing rel_ab
fct_seq_levels <- levels(fct_seq)

### Plot
p <- t %>%
  mutate(Genus = factor(Genus, levels = fct_Genus_levels)) %>%
...
# Subset
genus_summary_size <- table_rel_long %>%
  filter(seq %in% fct_seq_levels) %>%
  group_by(Genus, size.mm, duplicate.id) %>%
  summarise(sum_ab = sum(rel_ab)) 

# Test that the average is 100%
sum(genus_summary_size$sum_ab/nsamples(ps))
  
# Plot
p <- genus_summary_size %>%
  mutate(Genus = factor(Genus, levels = fct_Genus_levels)) %>%
...
# Save plot
fname <- "./results/genus/abund_sample.png"
ggsave(fname, plot = p)
...
# Save the plot
ggsave("./results/diversity.png", plot = p)
