...
```
# Define Size Indices
The following differential abundance analysis is going to do pairwise comparison of sizes.
There are six sizes of samples. 6 sizes, comparing 2 size classes at a time, creates 15 combinations.
I define the indices of these combinations here.

```{r define_pairs}
size_classes <- as.character(unique(metadata$size.mm))

# Get start and end indices for each class
group_indices <- lapply(0:(n_sizes - 1), function(i) {
  start <- i * n_replicates + 1
  end <- start + n_replicates - 1
  start:end
})
names(group_indices) <- size_classes 

# Generate pairwise combinations of size classes
size_pairs <- combn(size_classes, 2, simplify = FALSE)

# Get sample indices for each pair
pairwise_indices <- lapply(size_pairs, function(pair) {
  indices1 <- group_indices[[as.character(pair[1])]]
  indices2 <- group_indices[[as.character(pair[2])]]
  list(
    size_class_pair = pair,
...
# filter only top n_asv sequences
abs_filt <- table_abs %>%
  filter(seq %in% seq_names) 
  
# make the first column the new row names
if (is.character(abs_filt[[1]])) {
  rownames(abs_filt) <- abs_filt[[1]]  # Set row names using the first column
  abs_filt <- abs_filt[, -1]           # Remove the first column
}
...
#table_f_d1 <- abs_filt[,1:6]
#cond_f_d1 <- conds[1:6]


results_list <- list()

pair <- pairwise_indices[[1]]
#for (pair in pairwise_indices) {
  # Subset samples for the current pair
  pair_samples <- which(conds %in% pair)
  
  table_sub <- abs_filt[, pair_samples]
  cond_sub <- conds[pair_samples]
  
  # Run ALDEx2
  x.aldex <- aldex(table_sub, cond_sub, denom = "all", test = "t", effect = TRUE)
  
  # Identify significant features
  sig_feats <- rownames(x.aldex[x.aldex$we.eBH < 0.05 & abs(x.aldex$effect) > 1, ])
  
  if (length(sig_feats) == 0) next  # Skip if no sig features
  
  # Save for review
  results_list[[paste(pair, collapse = "_vs_")]] <- list(
    aldex = x.aldex,
...
#x.aldex <- aldex(table_f_d1, cond_f_d1, denom = "all", test = "t", effect = TRUE) # t-test

#sig_features <- x.aldex[x.aldex$we.eBH < 0.05, ]
##sig_features2 <- x.aldex[x.aldex$we.eBH < 0.05 & abs(x.aldex$effect) > 1, ]
