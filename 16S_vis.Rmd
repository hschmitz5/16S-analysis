...
# Genus + rel_ab + seq
genus_summary2 <- table_rel_long %>%
  group_by(Genus, Species, seq) %>%
  summarise(avg_ab = sum(rel_ab)/nsamples(ps))
...
order_idx <- order(genus_summary2$avg_ab, na.last = TRUE, decreasing = TRUE)  # indices
gs_sorted = genus_summary2[order_idx,]    
t <- head(gs_sorted,n_asv)

# Factor Genera
fct_Genus <- forcats::fct_infreq(t$Genus, t$avg_ab) # factor genus based on decreasing rel_ab
fct_levels <- levels(fct_Genus)
genus_names <- unique(t$Genus)

  p <- t %>%
       mutate(Genus = factor(Genus, levels = fct_levels)) %>%
       ggplot(aes(x = Genus, y = avg_ab)) + 
