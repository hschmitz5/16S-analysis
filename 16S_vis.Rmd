...
genus_summary_seq <- table_rel_long %>%
  group_by(Genus, Species, seq) %>%
  summarise(avg_ab = sum(rel_ab)/nsamples(ps), .groups = "drop") %>%  # average across samples
  arrange(desc(avg_ab))  # groups the matrix by the avg_ab

#### Subset the data to include n_asv ASVs:
t <- head(genus_summary_seq,n_asv)
...
### Factor seqs 
fct_seq <- forcats::fct_infreq(t$seq, t$avg_ab) # factor seq based on decreasing rel_ab
...
genus_summary_size <- table_rel_long %>%
  ...
  summarise(avg_ab = sum(rel_ab)/n_replicates) # average across replicates and combine different ASVs
  
# Plot
p <- genus_summary_size %>%
  mutate(Genus = factor(Genus, levels = fct_Genus_levels)) %>%
  ggplot(aes(x = Genus, y = avg_ab, fill = size.mm)) +
