...
n_asv         <- 300  # number of ASVs to include in the plot 
```
...
saveRDS(output, file = "./results/ancombc2_output_300.rds")
```
```{r ancom2_process}
res_prim = output$res

#df_size = res_prim %>%
    #dplyr::select(taxon, contains("size.mm"))
...
lfc_long <- res_prim %>%
  ...
  mutate(contrast = gsub("lfc_size.mm.", "", contrast))
