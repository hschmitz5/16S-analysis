...
i <- 2  # 1 is seq
order_idx <- order(table_rel[,i], na.last = TRUE, decreasing = TRUE)  # indices
table_rel_ordered <- table_rel[order_idx, i]
plot(1:2500, log10(table_rel_ordered))  #, ylim = c(0, 1)

#order_idx <- order(table_rel[,i], na.last = TRUE, decreasing = TRUE)  # indices
#table_rel_ordered <- table_rel[order_idx, i]
#plot(1:2500, log10(table_rel_ordered))  #, ylim = c(0, 1)

test <- filter(table_rel_long, sample == "10A")
order_idx <- order(test$rel_ab, na.last = TRUE, decreasing = TRUE)  # indices
test2 <- test[order_idx,] 
  
p <- head(test2,20) %>%
     ggplot(aes(x = forcats::fct_infreq(Genus, rel_ab), y = rel_ab)) +  # I think it's listing x in alphabetical order
    geom_point(
        shape = 21,
        color = "black",
        size = 3,
