...
genus_summary_seq <- table_rel_long %>%
  group_by(Genus, seq) %>%
  summarise(
    # mean is averaging across all samples (for each ASV)
    mean_ab = mean(rel_ab),   
    .groups = "drop") %>%  
  # groups the matrix by the mean_ab
  arrange(desc(mean_ab)) %>%  
  # Subset the data to include n_asv ASVs
  head(n_asv) %>%
  # Remove when Genus = NA
  filter(!is.na(Genus))
 
# Collapse separate seq
gs_seq_avg <- table_rel_long %>%
  # only include n_asv
  filter(seq %in% genus_summary_seq$seq) %>%
  ...
  mutate(Genus = forcats::fct_infreq(Genus, mean_ab))
  
### Define Genus factor
genus_levels <- levels(gs_seq_avg$Genus)
# Apply to genus_summary_seq
genus_summary_seq <- genus_summary_seq %>%
  mutate(Genus = factor(Genus, levels = genus_levels))

### Names
genus_names <- gs_seq_avg$Genus 
seq_names <- genus_summary_seq$seq

# Test that the average relative abundance adds up to 100% (don't apply head)
#sum(genus_summary_seq$mean_ab)
...
ggplot(gs_seq_avg, aes(x = fct_rev(Genus), y = mean_ab)) +
  geom_col(fill = "steelblue", width = 0.6) +
  geom_errorbar(aes(ymin = mean_ab - sd_ab, ymax = mean_ab + sd_ab), 
                width = 0.2, color = "black") +
  geom_point(
    data = genus_summary_seq,
...
# Plot  
ggplot(genus_summary_size, aes(x = fct_rev(Genus), y = mean_ab, fill = as.factor(size.mm))) +
  ...
  scale_fill_manual(values=met.brewer("Hiroshige", 6), name = "Size (mm)") +
  labs(
    x = "Genus",
    y = "Mean Relative Abundance [%]",
...
p <- genus_summary_size %>%
  ...
  geom_errorbar(aes(x=Genus, ymin=mean_ab-sd_ab, ymax=mean_ab+sd_ab)) + #, 
