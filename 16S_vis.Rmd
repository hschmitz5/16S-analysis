...
ps_filt2 = filter_taxa(ps_filt, function(x) sum(x > 3) > (0.2*length(x)), TRUE)

# relative abundance
ps_rel <- phyloseq::transform_sample_counts(ps_filt2, function(x) x*100/sum(x))  # convert to %
...
print(paste("Number of ASVs after filtering:", ntaxa(ps_filt2)))
```
```{r genus_abundance}
...
genus_summary2 <- table_rel_long %>% # perform some functions on table_rel
  group_by(Genus, size.mm, duplicate.id) %>% # group at the genus level, we will use this to sum up everything that has the same genus 
...
sum(genus_summary2[,4]/nsamples(ps))
```
...
### plot
gs.group_plot %>%
  ggplot(aes(x = size.mm, y = rel_ab)) +  #, fill = duplicate.id)) + 
  #ggplot(aes(x = size.mm, y = sum_ab)) +  #, fill = duplicate.id)) + 
